...
/////////////////////  MAIN  ////////////////////
int funcGtot_multi(int sch, int tst, double Tf, double a, double b, int nx, int Nmax, double CFL, int aff, int ind_cond, int iu, int Ntau, double dt_first, double Ttau, int cin_phase, int sch_cin_phase, int tst_multi, int tst_air,
  ...
  double Ctriple=1e-5; // Valeur de C sur le point triple (sinon C=0)
  ...
  // variables défini aux centres des mailles
    ...
    E[i]=W[1]*W[1]/2 + EPS[i];
	  ...
	  //printf("i=%d : ind_fluide= %d, TAU= %g, U= %g, E= %g, P= %g, RHO= %g\n",i,indFLUIDE[i],TAU[i],U[i],EPS[i],P[i],RHO0c[i] );
...
// debut de la boucle ============================================================================= 
  while((t<Tf)&&(n<Nmax)){
    ...
    for(int i=0; i<=N-1; i++){ 
    	//printf("i= %d, indFLUIDE= %d, ",i,indFLUIDE[i] );
    	dX=X[i+1]-X[i];
      EPS[i]= E[i]-U[i]*U[i]/2;  // energie interne 
      //printf(" E= %g, Etot= %g, U= %g, V= %g, Ustar= %g, Pstar= %g, Ustar1= %g, Pstar1= %g\n",EPS[i],E[i],U[i],TAU[i],Ustar[i],Pstar[i],Ustar[i+1],Pstar[i+1]  );
      /*
