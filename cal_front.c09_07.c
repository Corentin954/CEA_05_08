...
void fnb_pts(int nb_points, int* pNTG1, int* pNPB1, int* pNPB2, int* pNTD2 , int* pNTD3, int* pNPH3, int* pNTG3, 
  ...
  *pNTD2=4*nb_points;
  // C
  *pNTD3=ceil(nb_points/3);
  *pNPH3=6*nb_points;
  *pNTG3=4*nb_points; 
  
  *pN12=nb_points; *pN13=nb_points; *pN23=5*nb_points;
...
int ecr_benchmark_PT(int nb_points, int Nv ,int Ne, double Vmin, double Vmax, double Emin, double Emax){
    ...
    for(int j=0; j<Ne-1; j++){
      ...
      // ## 1 intersection donc seulment deux zones ##
        ...
        if(tabZONE[1][k]==12){
          ...
          printf("Erreur dans tabZONE= %d (ecr_benchmark_PT)\n",tabZONE[1][k]);
        }
      }
      ...
      // ## Plusieurs intersections ##
        ...
        for(int ie=0; ie<2; ie++){
          for(int iv=1; iv<N_rec-1; iv++){
            ...
            VE_front_rec[count][1]=Emin+(i+ie)*he;
            count++;
          }
        }
        ...
        for(int ir=0; ir<4*(N_rec-1); ir++){
          ...
          if(intPOLY(V_sub,E_sub,NC,SC)){ 
            ...
            err=graine_zone_mixte(V_sub, E_sub, N12, segVa12, segVb12, segEa12, segEb12, &Vmx, &Emx, &Vmy, &Emy);
            if(err){printf("err= %d (benchmark_PT)\n",err ); return err;}
            tabVx[k]=Vmx; tabEx[k]=Emx;
            tabVy[k]=Vmy; tabEy[k]=Emy;
            tabZONE[nb_zone][k]=12;
            nb_zone++;
          }
          else if(intPOLY(V_sub,E_sub,NAC,SAC)){
            err=graine_zone_mixte(V_sub, E_sub, N13, segVa13, segVc13, segEa13, segEc13, &Vmx, &Emx, &Vmy, &Emy);
            if(err){printf("err= %d (benchmark_PT)\n",err ); return err;}
            tabVx[k]=Vmx; tabEx[k]=Emx;
            tabVy[k]=Vmy; tabEy[k]=Emy;
            tabZONE[nb_zone][k]=13;
            nb_zone++;
          }
          else if(intPOLY(V_sub,E_sub,NBC,SBC)){
            err=graine_zone_mixte(V_sub, E_sub, N23, segVb23, segVc23, segEb23, segEc23, &Vmx, &Emx, &Vmy, &Emy);
            if(err){printf("err= %d (benchmark_PT)\n",err ); return err;}
            tabVx[k]=Vmx; tabEx[k]=Emx;
            tabVy[k]=Vmy; tabEy[k]=Emy;
            tabZONE[nb_zone][k]=23;
            nb_zone++;
          }
          else{  // hors domaine --> on ne se soucis des pb de chevauchement des rectangles sur les zones
            tabVx[k]=0; tabEx[k]=-1e-7;
  ...
  */
  ...
  if((fileZONE_0 = fopen("ZONEbench_0.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 0\n"); return 1;}
  if((fileZONE_1 = fopen("ZONEbench_1.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 1\n"); return 1;}
  if((fileZONE_2 = fopen("ZONEbench_2.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 2\n"); return 1;}
  if((fileZONE_3 = fopen("ZONEbench_3.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 3\n"); return 1;}
  if((fileZONE_4 = fopen("ZONEbench_4.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 4\n"); return 1;}
  ...
  FILE *file_Nb_ZONE;
  if((file_Nb_ZONE = fopen("Nb_ZONEbench.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 0\n"); return 1;}
  ...
  // fichier binaire Frontières de zones
  FILE *fileA,*fileB,*fileC,*fileAB,*fileAC,*fileBC;
  if((fileA = fopen("A.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 0\n"); return 1;}
  if((fileB = fopen("B.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 1\n"); return 1;}
  if((fileC = fopen("C.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 2\n"); return 1;}
  if((fileAB = fopen("AB.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 3\n"); return 1;}
  if((fileAC = fopen("AC.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 4\n"); return 1;}
  if((fileBC = fopen("BC.bin", "wb")) == NULL){printf("erreur ouverture fichier ZONE bench 4\n"); return 1;}
  
  // Ecriture des tableaux
  fwrite(SA,sizeof(double),NA+1,fileA);
  fwrite(SB,sizeof(double),NB+1,fileB);
  fwrite(SC,sizeof(double),NC+1,fileC);
  fwrite(SAB,sizeof(double),NAB+1,fileAB);
  fwrite(SAC,sizeof(double),NAC+1,fileAC);
  fwrite(SBC,sizeof(double),NBC+1,fileBC);

  fclose(fileA);  fclose(fileB);  fclose(fileC);
  fclose(fileAB);  fclose(fileAC);  fclose(fileBC);
...
*/
int lec_benchmark_PT(double V, double E, int nb_points, double** SA, double** SB, double** SC, double** SAB, double** SAC, double** SBC, double* tabVx, double* tabEx, double* tabVy, double* tabEy, int** tabZONE, int* tab_Nb_ZONE, int Nv, int Ne, double Vmin, double Vmax, double Emin, double Emax, double* pP, double* pT, double* px){
  int Nmax=1e4;
  double epsilon=1e-5;
  ...
  fnb_pts(nb_points, &NTG1, &NPB1, &NPB2, &NTD2 , &NTD3, &NPH3, &NTG3, &N12, &N13, &N23, &NA, &NB, &NC, &NAB, &NAC, &NBC );
   ...
   // ***********   CHECK POUR SAVOIR SI LES FRONTIERES SONT BIEN DEF   **************
   double sum=0;
   int idec=NTD3-1+NPH3-1+NTG3-2;
   for(int i=0; i<N13; i++){
     sum+=fabs(SA[i][0]-SC[idec+i][0]);
     sum+=fabs(SA[i][1]-SC[idec+i][1]);
   }
   printf("\n  **************\n");
   printf("    sum= %16lf\n",sum);
   printf("  **************\n\n");
  ...
  //  ##  2 zones   ## 
    ...
    for(int ind=0; ind<nb_zone; ind++){
      ...
      if(zone==3){
        ...
        else{printf("   out 3\n");} // commande pour passer au ind suivant
      }
      else if(zone==2){
        if(intPOLY(V,E,NB,SB)){
          P=fP(V,E,2);  T=fT(V,E,2);
          x=-1;
          return 0;
        }
...
int use_benchmark_PT(){
  ...
  FILE *fileZONE_0,*fileZONE_1,*fileZONE_2,*fileZONE_3,*fileZONE_4;
  if((fileZONE_0 = fopen("ZONEbench_0.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 0\n"); return 1;}
  if((fileZONE_1 = fopen("ZONEbench_1.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 1\n"); return 1;}
  if((fileZONE_2 = fopen("ZONEbench_2.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 2\n"); return 1;}
  if((fileZONE_3 = fopen("ZONEbench_3.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 3\n"); return 1;}
  if((fileZONE_4 = fopen("ZONEbench_4.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 4\n"); return 1;}
  ...
  // fichier binaire NOMBRE DE ZONE
  FILE *file_Nb_ZONE;
  if((file_Nb_ZONE = fopen("Nb_ZONEbench.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 0\n"); return 1;}
  ...
  fclose(file_Nb_ZONE);
   ...
   // fichier binaire Frontières de zones
  FILE *fileA,*fileB,*fileC,*fileAB,*fileAC,*fileBC;
  if((fileA = fopen("A.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 0\n"); return 1;}
  if((fileB = fopen("B.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 1\n"); return 1;}
  if((fileC = fopen("C.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 2\n"); return 1;}
  if((fileAB = fopen("AB.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 3\n"); return 1;}
  if((fileAC = fopen("AC.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 4\n"); return 1;}
  if((fileBC = fopen("BC.bin", "rb")) == NULL){printf("erreur ouverture fichier ZONE bench 4\n"); return 1;}
  ...
  // Ecriture des tableaux
  fread(SA,sizeof(double),NA+1,fileA);
  fread(SB,sizeof(double),NB+1,fileB);
  fread(SC,sizeof(double),NC+1,fileC);
  fread(SAB,sizeof(double),NAB+1,fileAB);
  fread(SAC,sizeof(double),NAC+1,fileAC);
  fread(SBC,sizeof(double),NBC+1,fileBC);
  ...
  for(int i=0; i<Nv_test; i++){
    ...
    for(int j=0; j<Ne_test; j++){
      ...
      err=lec_benchmark_PT(V, E, nb_points, SA, SB, SC, SAB, SAC, SBC, tabVx, tabEx, tabVy, tabEy, tabZONE, tab_Nb_ZONE, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis, &P, &T, &x);
...
int main(){
  ...
  */
  ...
  // Test de inter_seg
/*
  double** REC=alloctabd(5,2); // carré
  REC[0][0]=0.;        REC[0][1]=0.;
  REC[1][0]=0.;        REC[1][1]=1.;
  REC[2][0]=1.;        REC[2][1]=1.;
  REC[3][0]=1.;        REC[3][1]=0.;
  ...
  int N=3;
  double** S=alloctabd(N+1,2); // carré
  S[0][0]=1./2.;    S[0][1]=0;
  S[1][0]=3./2;     S[1][1]=1./2;
  S[2][0]=5./2;     S[2][1]=-3./2;
  S[3][0]=S[0][0];  S[3][1]=S[0][1];
  
  int res=inter_seg(REC, N, S);
  if(res==1){printf("Intersection\n");}
  else if(res==0){printf("pas d'intersection\n");}
  else {printf("errer\n"); return res;}
*/
}
