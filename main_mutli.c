	  ...
	  if (argv[i][0] == '-'){
	  	switch(argv[i][1]){
	  	  case 'h' : printf("Usage : \n"); 
                   ...
                   printf(" -r intialisation des fluides sur les mailles \n");
                   printf("                    0 : 1/6 du domaine de chaque coté contient de l'air, le reste est de l'étain   \n");
...
// ##############################################################################
  ...
  int deux_plaques; // si choc_double==0 --> choc simple sinon choc double 
  if(tst==100){deux_plaques=0;}
  else if(tst>100){deux_plaques=1;}
  ...
  if(Rsch==0){
    err=funcGtot_etain(sch, tst, T, a, b, nx, Nmax, CFL, aff, ind_cond, iu, Ntau, dt_first, Ttau, cin_phase, sch_cin_phase, tst_multi,
    ...
    err=funcRKint_etain(sch, tst, T, a, b, nx, Nmax, CFL, aff, ind_cond, iu, Ntau, dt_first, Ttau, cin_phase, sch_cin_phase, tst_multi, q, Cq, Cl, z, dpi, so,
                  nb_points, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis, tabZONE, tab_Nb_ZONE, 
                  tabVx, tabEx, tabVy, tabEy, SA, SB, SC, SAB, SAC, SBC);
    if(err){print_err(err,Rsch, sch); return 1;}
  }
    ...
    if(sch==0){
      err=funcBBC_JCP2009_etain(tst, T, a, b, nx, Nmax, CFL, aff, ind_cond, iu, Ntau, dt_first, Ttau, cin_phase, sch_cin_phase, tst_multi, q, Cq, Cl,
                          nb_points, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis,  tabZONE, tab_Nb_ZONE, 
      ...
      err=funcBBC_PRED_CORR_etain(tst, T, a, b, nx, Nmax, CFL, aff, ind_cond, iu, Ntau, dt_first, Ttau, cin_phase, sch_cin_phase, tst_multi, q, Cq, Cl,
                            nb_points, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis, tabZONE, tab_Nb_ZONE, 
                            tabVx, tabEx, tabVy, tabEy, SA, SB, SC, SAB, SAC, SBC);
      if(err){print_err(err,Rsch, sch); return 1;}
    }
      ...
      err=funcBBC_RK2av_etain(tst, T, a, b, nx, Nmax, CFL, aff, ind_cond, iu, Ntau, dt_first, Ttau, cin_phase, sch_cin_phase, tst_multi, q, Cq, Cl,
                        nb_points, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis, tabZONE, tab_Nb_ZONE, 
                        tabVx, tabEx, tabVy, tabEy, SA, SB, SC, SAB, SAC, SBC);
      if(err){print_err(err,Rsch, sch); return 1;}
    }
    ...
    err=funcvNR_etain(tst, T, a, b, nx, Nmax, CFL, aff, ind_cond, iu, Ntau, dt_first, Ttau, cin_phase, sch_cin_phase, tst_multi, q, Cq, Cl,
                nb_points, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis, tabZONE, tab_Nb_ZONE, 
                tabVx, tabEx, tabVy, tabEy, SA, SB, SC, SAB, SAC, SBC);
    if(err){print_err(err,Rsch, sch); return 1;}
  }
    ...
    err=funcKOVA_etain(tst, T, a, b, nx, Nmax, CFL, aff, ind_cond, iu, Ntau, dt_first, Ttau, cin_phase, sch_cin_phase, tst_multi, z,
                 nb_points, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis, tabZONE, tab_Nb_ZONE, 
                 tabVx, tabEx, tabVy, tabEy, SA, SB, SC, SAB, SAC, SBC);
    if(err){print_err(err,Rsch, sch); return 1;}
  }
