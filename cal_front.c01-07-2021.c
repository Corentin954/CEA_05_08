...
*/
int sommetsAB(int N12, int ND, int NG, double*P12, double* T12, double** S){
  

  int N=N12+(ND-1)+(N12-1)+(NG-2) ;
  ...
  double VDA, EDA, VDB, EDB;
  err= fVE(phaseA, P12[0], T12[0], &VDA, &EDA);
  if(err){printf("Erreur de fVE (sommetsAB)\n"); return err;}
  err= fVE(phaseB, P12[0], T12[0], &VDB, &EDB);
  if(err){printf("Erreur de fVE (sommetsAB)\n"); return err;}
  double hVD=(VDB-VDA)/(ND-1);
  double hED=(EDB-EDA)/(ND-1);

  double VGA, EGA, VGB, EGB;
  err= fVE(phaseA, P12[N12-1], T12[N12-1], &VGA, &EGA);
  if(err){printf("Erreur de fVE (sommetsAB)\n"); return err;}
  err= fVE(phaseB, P12[N12-1], T12[N12-1], &VGB, &EGB);
  if(err){printf("Erreur de fVE (sommetsAB)\n"); return err;}
  double hVG=(VGA-VGB)/(NG-1);
  double hEG=(EGA-EGB)/(NG-1);
  ...
  for(int j=1; j<=N12-1; j++){ // N13 points
    T=T12[N12-1-j];
    P=P12[N12-1-j];
    err= fVE(phaseA, P, T, &V, &E);
    if(err){printf("Erreur de fVE (sommetsC)\n");return err;}
    S[nref+j-1][0]=V;
    S[nref+j-1][1]=E;
  }
...
/* Arguments :
  ...
  - double** S : les sommets du polygone tendant vers la zone de mélange 12
*/
int sommetsAC(int N13, int ND, int NG, double*P13, double* T13, double** S){
  

  int N=N13+(ND-1)+(N13-1)+(NG-2) ;
  ...
  double VDA, EDA, VDC, EDC;
  err= fVE(phaseA, P13[N13-1], T13[N13-1], &VDA, &EDA);
  if(err){printf("Erreur de fVE (sommetsAB)\n"); return err;}
  err= fVE(phaseC, P13[N13-1], T13[N13-1], &VDC, &EDC);
  if(err){printf("Erreur de fVE (sommetsAB)\n"); return err;}
  double hVD=(VDA-VDC)/(ND-1);
  double hED=(EDA-EDC)/(ND-1);

  double VGA, EGA, VGC, EGC;
  err= fVE(phaseA, P13[0], T13[0], &VGA, &EGA);
  if(err){printf("Erreur de fVE (sommetsAB)\n"); return err;}
  err= fVE(phaseC, P13[0], T13[0], &VGC, &EGC);
  if(err){printf("Erreur de fVE (sommetsAB)\n"); return err;}
  double hVG=(VGC-VGA)/(NG-1);
  double hEG=(EGC-EGA)/(NG-1);
  ...
  for(int j=0; j<=N13-1; j++){ 
    T=T13[j];
    P=P13[j];
    err= fVE(phaseC, P, T, &V, &E);
    if(err){printf("Erreur de fVE (sommetsAC)\n");return err;}
    S[nref+j][0]=V;
    S[nref+j][1]=E;
  }
  ...
  //  Parcours de C13 de la gauche jusqu'au point triple 
  for(int j=1; j<=N13-1; j++){ // N13 points
    T=T13[N13-1-j];
    P=P13[N13-1-j];
    err= fVE(phaseA, P, T, &V, &E);
    if(err){printf("Erreur de fVE (sommetsAC)\n");return err;}
...
/* Arguments :
  ...
  - double** S : les sommets du polygone tendant vers la zone de mélange 12
*/
int sommetsBC(int N23, int ND, int NG, double*P23, double* T23, double** S){
  
  int N=N23+(ND-1)+(N23-1)+(NG-2);
  ...
  double VDB, EDB, VDC, EDC;
  err= fVE(phaseB, P23[0], T23[0], &VDB, &EDB);
  if(err){printf("Erreur de fVE (sommetsBC)\n"); return err;}
  err= fVE(phaseC, P23[0], T23[0], &VDC, &EDC);
  if(err){printf("Erreur de fVE (sommetsBC)\n"); return err;}
  double hVD=(VDC-VDB)/(ND-1);
  double hED=(EDC-EDB)/(ND-1);
  

  double VGB, EGB, VGC, EGC;
  err= fVE(phaseB, P23[N23-1], T23[N23-1], &VGB, &EGB);
  if(err){printf("Erreur de fVE (sommetsBC)\n"); return err;}
  err= fVE(phaseC, P23[N23-1], T23[N23-1], &VGC, &EGC);
  if(err){printf("Erreur de fVE (sommetsBC)\n"); return err;}
  double hVG=(VGB-VGC)/(NG-1);
  double hEG=(EGB-EGC)/(NG-1);
  ...
  //  Parcours de C13 de la gauche jusqu'au point triple 
  for(int j=1; j<=N23-1; j++){ // N13 points
    T=T23[N23-1-j];
    P=P23[N23-1-j];
    err= fVE(phaseB, P, T, &V, &E);
    if(err){printf("Erreur de fVE (sommetsC)\n");return err;}
...
int sommets_polygone(int NA, int NB, int NC, int NTG1, int NPB1, int NPB2, int NTD2, int NTD3, int NPH3, int NTG3, int N12, int N13, int N23, double ** SA, double** SB, double** SC, 
                     int NGAB, int NDAB, int NGAC, int NDAC, int NGBC, int NDBC, double** SAB, double** SAC, double** SBC,
...
*/
  ...
  printf("Sommets AB\n");
  err=sommetsAB(N12, NDAB, NGAB, segP12, segT12, SAB);
  printf("Sommets AC\n");
  err=sommetsAC(N13, NDAC, NGAC, segP13, segT13, SAC);
  printf("Sommets BC\n");
  err=sommetsBC(N23, NDBC, NGBC, segP23, segT23, SBC);
...
int diag_zone(){
  ...
  int NGAB=ceil(nb_points/2); int NDAB=ceil(nb_points/2);
  int NAB=N12+(NDAB-1)+(N12-1)+(NGAB-2);
  double** SAB=alloctabd(NAB+1,2);

  int NGAC=ceil(nb_points/2); int NDAC=ceil(nb_points/2);
  int NAC=N13+(NDAC-1)+(N13-1)+(NGAC-2);
  double** SAC=alloctabd(NAC+1,2);

  int NGBC=ceil(nb_points/2); int NDBC=ceil(nb_points/2);
  int NBC=N23+(NDBC-1)+(N23-1)+(NGBC-2);
  ...
  */
  ...
  err=sommets_polygone(NA, NB, NC, NTG1, NPB1, NPB2, NTD2, NTD3, NPH3, NTG3, N12, N13, N23, SA, SB, SC, NGAB, NDAB, NGAC, NDAC, NGBC, NDBC, SAB, SAC, SBC, segP12, segT12, segP13, segT13, segP23, segT23);
...
*/
  int interp_zone_mixte(double V, double E, int phaseALPHA, int phaseBETA, int N, double* Valpha, double* Vbeta, double* Ealpha, double* Ebeta, double* pP, double* pT){
    ...
    if(testNO){printf(" le point est dans aucun des quadrilatere\n"); return 1;}
