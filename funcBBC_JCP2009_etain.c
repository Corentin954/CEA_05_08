...
/////////////////////  MAIN  ////////////////////
int funcBBC_JCP2009_etain(int tst, double Tf, double a, double b, int nx, int Nmax, double CFL, int aff, int ind_cond, int iu, int Ntau, double dt_first, double Ttau, int cin_phase, int sch_cin_phase, int tst_multi, int q, double Cq, double Cl,
...
// debut de la boucle ============================================================================= 
  while((t<Tf)&&(n<Nmax)){
    ...
    if(etain){
      for(int i=0; i<=N-1; i++){
        dX=X[i+1]-X[i];
        //EGS(tst,TAU2[i], EPS2[i], &P2[i], &C2[i]);
        /*
        err=fPTC(TAU2[i], EPS2[i], &P2[i], &C2[i], &T[i], &FM[i],
                 nb_points, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis, 
                 SA, SB, SC, SAB, SAC, SBC, tabVx, tabEx, tabVy, tabEy, tabZONE, tab_Nb_ZONE );
        */
