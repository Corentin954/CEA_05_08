...
*/
int inter_seg(double** REC, int N, double** S){
        ...
        if(xi1==xi){ // pas de pente pour le segment S
          ...
          */
          ...
          if(penteP==pente){} // segment parralelle //
            ...
            if( (xI-xa)*(xI-xb)<0 &&  (yI-ya)*(yI-yb)<0  &&  (xI-xi)*(xI-xi1)<0 ){return 1;}
          }
          else if(ya==yb){
            if( (xI-xa)*(xI-xb)<0 &&  (xI-xi)*(xI-xi1)<0 &&  (yI-yi)*(yI-yi1)<0 ){return 1;}
          }
          else{
            if( (xI-xa)*(xI-xb)<0 &&  (yI-ya)*(yI-yb)<0  &&  (xI-xi)*(xI-xi1)<0 &&  (yI-yi)*(yI-yi1)<0 ){return 1;}
          }
...
int ecr_benchmark_PT(int Nv ,int Ne, double Vmin, double Vmax, double Emin, double Emax){
  ...
  int* tabZONE=malloc((Nv-1)*(Ne-1)*sizeof(int));
  //int** tabZONE=alloctab((Nv-1)*(Ne-1),4);
...
int lec_benchmark_PT(double V, double E, double* tabVx, double* tabEx, double* tabVy, double* tabEy, int* tabZONE, int Nv, int Ne, double Vmin, double Vmax, double Emin, double Emax, double* pP, double* pT, double* px){
  ...
  */
  ...
  else{printf("Erreur dans le choix de ind_err (lec_benchmark_PT)\n"); }
...
int use_benchmark_PT(){
  ...
  rewind(fileZONE); 
  int* tabZONE=malloc((Nv_dis-1)*(Ne_dis-1)*sizeof(int));
  fread(tabZONE,sizeof(int),(Nv_dis-1)*(Ne_dis-1),fileZONE);
  fclose(fileZONE);
