...
/////////////////////  MAIN  ////////////////////
int funcRKint_multi(int sch, int tst, double Tf, double a, double b, int nx, int Nmax, double CFL, int aff, int ind_cond, int iu, int Ntau, double dt_first, double Ttau, int cin_phase, int sch_cin_phase, int tst_multi, int tst_air, int q, double Cq, double Cl, int z, int dpi, int so,
  ...
  if(z==0 || z==1){}
  else{printf("Erreur z= %d (kinetic energy fix)\n",z ); return 1;}
  if(dpi==0 || dpi==1){}
  else{printf("Erreur dpi= %d\n",dpi ); return 1;}
  ...
  if(so==2 || so==3){}//juste pour verif les valeurs
  else if(so==5){so=4;}
  else if(so==7){so=5;}
  else if(so==9){so=6;}
  else{printf("Erreur so= %d (funcRKint_etain)\n",so); return 1;}
  ...
  */
  ...
  double* W=malloc(10*sizeof(double));
    ...
    matFM[i][1]=W[6];
    matFM[i][2]=W[7];
    //printf(" i= %d | xA= %g, xB= %g, xC= %g\n",i,matFM[i][0],matFM[i][1],matFM[i][2] );

    if(indFLUIDE[i]==ind_etain){
      err=epsilon_etain(1./W[0], W[2], T[i], &EPSpw, nb_points, SA, SB, SC, SAB, SAC, SBC );
      if(err){return err;}
      REPSpw[i]=W[0]*EPSpw;
    }
    else if(indFLUIDE[i]==ind_air){
      REPSpw[i]=W[0]*epsilonEOS(tst_air, 1./W[0], W[2]);
    }
  ...
  for(int i=0; i<=N-1; i++){
    ...
    /*
    if(indFLUIDE[i]==ind_etain){ 
      err=fPTC_cin_phase(0, 0, sch_cin_phase, RTAUpw[i]/RHO0c[i], REPSpw[i]/RHO0c[i], &P[i], &C[i], &T[i], matFM[i], matFM[i], Ntau, dx, dt, Ttau, &zone,
    ...
    */
    if(indFLUIDE[i]==ind_air){
      EGS(tst_air,RTAUpw[i]/RHO0c[i], REPSpw[i]/RHO0c[i], &P[i], &C[i]);
    }
    XsC[i]=(X[i+1]-X[i])/C[i];
  ...
  for(int i=0; i<=N-1; i++){
    printf("i=%d : ind_fluide= %d, TAU= %g, E= %g, P= %g, C= %g, T= %g, U= %g\n",i,indFLUIDE[i], RTAUpw[i]/RHO0c[i], REPSpw[i]/RHO0c[i], P[i], C[i], T[i], RUpw[i]/RHO0d[i] );
  }
...
// debut de la boucle ============================================================================= 
    ...
    for(int k=0; k<to; k++){
      ...
      for(int i=nbghosts-nbghosts2; i<=N2-1; i++){  ///
  	  ...
  	  for(int i=nbghosts-nbghosts2; i<=N2-1; i++){ ////
        printf(" k= %d to= %d, i= %d, ind_fluide= %d\n",k,to,i,indFLUIDE[i] );
        printf("V= %g, E= %g, U= %g ",TAUstar[k][i], EPSstar[k][i], Ustar[k][i] );

        dX=Xstar[k][i+1]-Xstar[k][i];

        if(indFLUIDE[i]==ind_etain){ 
          err=fPTC_cin_phase(0, cin_phase, sch_cin_phase, TAUstar[k][i], EPSstar[k][i], &Pstar[k][i], &Cstar[k][i], &T[i], matFM[i], lambda_out_unuse, Ntau, dX, A[k][k]*dt, Ttau, &zone,
                 nb_points, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis, 
                 SA, SB, SC, SAB, SAC, SBC, tabVx, tabEx, tabVy, tabEy, tabZONE, tab_Nb_ZONE );
          if(err){return err;}
        }
    ...
    for(int i=nbghosts-nbghosts2; i<=N2-1; i++){ //// 
      printf(" -n= %d, i= %d, ind_fluide= %d, V= %g, E= %g, U= %g ",n,i,indFLUIDE[i],RTAUpw[i]/RHO0c[i], REPSpw[i]/RHO0c[i],RUpw[i]/RHO0d[i] );
      dX=X[i+1]-X[i];
      if(indFLUIDE[i]==ind_etain){ 
        err=fPTC_cin_phase(0, cin_phase, sch_cin_phase, RTAUpw[i]/RHO0c[i], REPSpw[i]/RHO0c[i], &P[i], &C[i], &T[i], matFM[i], matFM[i], Ntau, dX, dt, Ttau, &zone,
                              nb_points, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis, 
                              SA, SB, SC, SAB, SAC, SBC, tabVx, tabEx, tabVy, tabEy, tabZONE, tab_Nb_ZONE );
        if(err){return err;}
      }
