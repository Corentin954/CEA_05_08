...
/////////////////////  MAIN  ////////////////////
int funcBBC_PRED_CORR(int tst, double T, double a, double b, int nx, int Nmax, double CFL, int aff, int cond_lim, int iu, int q , double Cq, double Cl){
...
// debut de la boucle ============================================================================= 
	while((t<T)&&(n<Nmax)){
    // cond aux limites     
    err=condlim(cond_lim, ideb,ifin, nbghosts, RHO, 0);
    if(err){return err;}
    err=condlim(cond_lim, ideb,ifin, nbghosts, EPS, 0);
    if(err){return err;}
    err=condlim(cond_lim, ideb,ifin+1, nbghosts, U, iu);
    if(err){return err;}
    ...
    err=condlim(cond_lim, ideb,ifin+1, nbghosts, U1, iu);
    if(err){return err;}

    for(int i=ideb; i<=ifin+1; i++){
      U2pr[i]=(U[i] + U1[i])/2;           
    }
    err=condlim(cond_lim, ideb,ifin+1, nbghosts, U2pr, iu);
    if(err){return err;}
    ...
    err=condlim(cond_lim, ideb,ifin, nbghosts, RHO1, 0);
    if(err){return err;}
    err=condlim(cond_lim, ideb,ifin, nbghosts, EPS1, 0);
    if(err){return err;}
    ...
    err=condlim(cond_lim, ideb,ifin+1, nbghosts, U, iu);
    if(err){return err;}
    err=condlim(cond_lim, ideb,ifin+1, nbghosts, U2, iu);
    if(err){return err;}
