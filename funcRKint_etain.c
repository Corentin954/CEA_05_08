...
/////////////////////  MAIN  ////////////////////
int funcRKint_etain(int sch, int tst, double Tf, double a, double b, int nx, int Nmax, double CFL, int aff, int ind_cond, int iu, int Ntau, double dt_first, double Ttau, int cin_phase, int sch_cin_phase, int tst_multi, int q, double Cq, double Cl, int z, int dpi, int so,
  ...
  */
  ...
  if(etain==1){
    for(int i=0; i<=N-1; i++){
      ...
      /*
      err=fPTC(RTAUpw[i]/RHO0c[i], REPSpw[i]/RHO0c[i], &P[i], &C[i], &T[i], &FM[i],
...
// debut de la boucle ============================================================================= 
    ...
    for(int k=0; k<to; k++){
      ...
      if(etain){
          ...
          /*
          err=fPTC(TAUstar[k][i], EPSstar[k][i], &Pstar[k][i], &Cstar[k][i], &T[i], &FM[i],
               nb_points, Nv_dis, Ne_dis, Vmin_dis, Vmax_dis, Emin_dis, Emax_dis, 
               SA, SB, SC, SAB, SAC, SBC, tabVx, tabEx, tabVy, tabEy, tabZONE, tab_Nb_ZONE );
          */
